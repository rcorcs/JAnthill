...
public class DefaultEnvironment extends Environment {
	...
	public int instantiate(String hostName, ModuleInfo moduleInfo, int taskId){
		...
		String cmd = "java -cp "+anthillPath+":"+appPath+" "+Settings.getClassName()+" -tid "+taskId+" -app-xml "+appXml+" -m "+moduleInfo.getName()+" -xml "+xml+" -h "+hostName+" -sa "+WebServerSettings.getAddress()+" -sp "+WebServerSettings.getPort()+" > "+hostInfo.getWorkspace()+AppSettings.getName()+"/"+(moduleInfo.getName()+"-tid"+taskId)+".log";
