...
public class DefaultEnvironment extends Environment {
	...
	public int instantiate(String hostName, ModuleInfo moduleInfo, int taskId){
		...
		String cmd = "java -cp "+anthillPath+":"+appPath+" "+Settings.getClassName()+" -tid "+taskId+" -app-xml "+appXml+" -m "+moduleInfo.getName()+" -xml "+xml+" -h "+hostName+" -sa localhost -sp 8080"+" > "+hostInfo.getWorkspace()+AppSettings.getName()+"/"+(moduleInfo.getName()+"-tid"+taskId)+".log";
